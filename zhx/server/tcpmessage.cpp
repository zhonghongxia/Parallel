...
void tcp_message::decode_to_my_message(struct my_message &msg)	
{
	...
	char *file_content = data_[header_len+KEEPON_LEN+FILECONTENTLEN_LEN + SOURCEID_LEN+FILENAME_LEN];
}
void tcp_message::encode_my_message(struct my_message &msg )
{		
	clear();
	...
	memcpy(data_+FILENAME_LEN , msg.result,msg.result_len);
}
