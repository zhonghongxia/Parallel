...
float con_time[CLI_NUM]; 
	...
	for (i = 0; i < CLI_NUM; i++) 
	{
		...
		*(int *)pack[i].write_buf = BODY_LEN;
		sprintf(pack[i].write_buf+HEAD_LEN , "%s" , "AAAA");
		pack[i].write_ptr = pack[i].write_buf;

		pack[i].read_buf = (char *)calloc(1 ,BODY_LEN+HEAD_LEN);			
	...
	while(count < CLI_NUM)
	{
		...
		for(i = 0; i < epoll_ready_num ;i++)
		{
			...
			if(events[i].events & EPOLLIN)
			{
				...
				ptr = (PACK *)events[i].data.ptr;
                int nread;
				while(( nread = read ( ptr -> sock_fd ,ptr ->read_ptr, &ptr->read_buf[BODY_LEN+HEAD_LEN] - ptr ->read_ptr )) > 0 )
                {
					ptr ->read_ptr += nread;
				}
				if(nread < 0)
		...
		if( ptr -> read_ptr  - ptr -> read_buf == BODY_LEN+
HEAD_LEN)
                {
